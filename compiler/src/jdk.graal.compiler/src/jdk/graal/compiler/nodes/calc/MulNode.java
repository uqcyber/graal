...
public class MulNode extends BinaryArithmeticNode<Mul> implements NarrowableArithmeticNode, Canonicalizable.BinaryCommutative<ValueNode> {
    ...
    static {
        ...
        useGenerated = enabledNodes.contains("MulNode");
    }
    ...
    private static ValueNode canonicalizeGenerated(MulNode self, ValueNode x, ValueNode y) { //canonicalizeGenerated
        if (x instanceof MulNode ec) {
            var a = ec.getX();
            var b = ec.getY();
            if (a instanceof NegateNode ac) {
                ...
                if (b instanceof NegateNode bc) {
                    var azz = bc.getValue();
                    return new MulNode(az, azz);
                }
            }
        }
        return self != null ? self : new MulNode(x, y).maybeCommuteInputs();
    }

    public ValueNode canonicalGenerated(CanonicalizerTool tool, ValueNode forX, ValueNode forY) { //canonicalGenerated
        ValueNode ret = super.canonical(tool, forX, forY);
        if (ret != this) {
            return ret;
        }
