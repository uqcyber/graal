...
public class BigIntegerLeftShiftWorkerNode extends MemoryKillStubIntrinsicNode {

    public static final NodeClass<BigIntegerLeftShiftWorkerNode> TYPE = NodeClass.create(BigIntegerLeftShiftWorkerNode.class);
    public static final LocationIdentity[] KILLED_LOCATIONS = {NamedLocationIdentity.getArrayLocation(JavaKind.Int)};

    public static final ForeignCallDescriptor STUB = new ForeignCallDescriptor("bigIntegerLeftShiftWorker", void.class,
    ...
    public BigIntegerLeftShiftWorkerNode(ValueNode newArr, ValueNode oldArr, ValueNode newIdx, ValueNode shiftCount, ValueNode numIter) {
        this(newArr, oldArr, newIdx, shiftCount, numIter, null);
    }

    public BigIntegerLeftShiftWorkerNode(ValueNode newArr, ValueNode oldArr, ValueNode newIdx, ValueNode shiftCount, ValueNode numIter, EnumSet<?> runtimeCheckedCPUFeatures) {
    ...
    @NodeIntrinsic
    @GenerateStub(name = "bigIntegerLeftShiftWorker", minimumCPUFeaturesAMD64 = "minFeaturesAMD64")
    ...
    public void emitIntrinsic(NodeLIRBuilderTool gen) {
        gen.getLIRGeneratorTool().emitBigIntegerLeftShiftWorker(gen.operand(newArr), gen.operand(oldArr), gen.operand(newIdx), gen.operand(shiftCount), gen.operand(numIter));
    }
}
