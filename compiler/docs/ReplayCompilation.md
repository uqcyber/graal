...
The GraalVM compiler can record the inputs to a compilation task, serialize these inputs into a replay file, and
reproduce the compilation using the same inputs. Replay compilation is based on instrumenting the JVM Compiler
Interface (JVMCI). Truffle compilations are currently not supported. It is not a goal to execute the replayed code.
...
The below command records and serializes every compilation into `./replay-files/replaycomp/<compile-id>.replay`.
The directories are created if they do not exist. To use the legacy JSON format instead, use
`-Djdk.graal.ReplayFileFormat=Json`.
...
also accepts a path to a single file.
...
The command below exercises this behavior by forcing a crash. The replay file can then be found in the `replaycomp`
...
native launcher. With the below commands, all replay related processing (including replay-file parsing) is performed by
libgraal code.
...
included in the replay file. If there is a mismatch, the command exits with a non-zero status.
...
recorded in the replay file. The default behavior is to return default values, which may not cause a compiler crash.
